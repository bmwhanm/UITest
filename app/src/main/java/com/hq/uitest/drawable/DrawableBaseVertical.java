...
public class DrawableBaseVertical extends Drawable{
    private Paint mPaint;
    private BitmapDrawable mSelectedDrawable,mUnSelectedDrawable;

    public DrawableBaseVertical(BitmapDrawable selectedDrawable, BitmapDrawable unSelectedDrawable){
        mPaint = new Paint();
...
//        Log.e("TAG","level  :  "+level);
        ...
        if(level > 0 && level < 10000){
            ...
            canvas.save();
            canvas.clipRect(0,0,mSelectedWidth  ,mUnSelectedHeight );
            canvas.drawBitmap(mUnSelectedDrawable.getBitmap(),0,0, mPaint);
            canvas.restore();


            canvas.save();
            canvas.clipRect(0,mSelectedHeight * (1 - ratio),mSelectedWidth   ,mSelectedHeight );
            canvas.drawBitmap(mSelectedDrawable.getBitmap(),0,0, mPaint);
            canvas.restore();
